...
impl Platform {
    ...
    ///
    /// This sits between the two primitives above. [`compress_in_place`](Self::compress_in_place)
    /// takes a runtime `block_len`, but handles only a single message.
    /// [`hash_many`](Self::hash_many) compresses many messages at once, but always assumes full
    /// blocks -- `block_len` isn't part of its interface on any backend, including the hand-written
    /// assembly. This one does both, which is what lets a batch of independent inputs finalize
    /// their short final blocks in parallel instead of one at a time. See [`crate::many`].
    ///
    ...
    /// always parallel.
    ///
    /// `cvs` and `blocks` must have the same length. This is debug-asserted rather
    /// than checked, like the `out` length in [`hash_many`](Self::hash_many).
    #[inline]
