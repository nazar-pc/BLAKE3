...
//! [`hash`](crate::hash), [`keyed_hash`](crate::keyed_hash),
//! [`derive_key`](crate::derive_key), or [`Hasher`](crate::Hasher) in any way, and hashing a
//! single input with one of them always produces exactly the same output as the corresponding
//! single-input function. What they add is the ability to hash a batch of same-length,
//! independent inputs (for example, the leaves of a Merkle tree) while sharing the cost of
//! SIMD parallelism across the whole batch, rather than paying it once per input.
//!
...
//! root-finalized directly. For a longer input, this module recurses the way
//! [`hash`](crate::hash) itself does internally, splitting at [`hazmat::left_subtree_len`] to
//! build the same BLAKE3 tree, except every node of that recursion -- each chunk, and each
//! parent joining two subtrees -- is computed for every input in the batch together. Because
//! the inputs share a length they share a tree shape, so the recursion stays in lockstep.
//!
...
//! one BLAKE3 ever allows to be short -- goes through
//! [`Platform::compress_many`](crate::platform::Platform::compress_many), the multi-lane
//! primitive that does take a runtime block length. Short inputs therefore get the same
//! cross-input SIMD parallelism as long ones.
...
mod test {
    ...
    fn test_many_mixed_content_arbitrary_lengths() {
        ...
        let mut input_buf = vec![0u8; 3 * MAX_SIMD_DEGREE * CHUNK_LEN];
        paint_test_input(&mut input_buf);
        for &len in &[
            0,
            1,
            ...
            let inputs: Vec<&[u8]> = (0..2 * MAX_SIMD_DEGREE + 1)
                .map(|i| &input_buf[i * 7 % (input_buf.len() - len)..][..len])
                .collect();
            check(&inputs, None, None);
        }
    }
