...
fn bench_many_hash(b: &mut Bencher, len: usize) {
...
fn bench_many_hash_loop(b: &mut Bencher, len: usize) {
    let mut inputs = many_inputs(b, len);
    let mut out = [blake3::Hash::from_bytes([0; OUT_LEN]); MANY_BATCH];
    b.iter(|| {
...
#[bench]
fn bench_many_hash_0032_bytes(b: &mut Bencher) {
    bench_many_hash(b, 32);
}

#[bench]
fn bench_many_hash_0032_bytes_loop(b: &mut Bencher) {
    bench_many_hash_loop(b, 32);
}

#[bench]
fn bench_many_hash_0001_block(b: &mut Bencher) {
    bench_many_hash(b, BLOCK_LEN);
}

#[bench]
fn bench_many_hash_0001_block_loop(b: &mut Bencher) {
    bench_many_hash_loop(b, BLOCK_LEN);
}
...
#[bench]
fn bench_many_hash_0100_bytes(b: &mut Bencher) {
    bench_many_hash(b, 100);
}

#[bench]
fn bench_many_hash_0100_bytes_loop(b: &mut Bencher) {
    bench_many_hash_loop(b, 100);
}

#[bench]
fn bench_many_hash_0001_kib(b: &mut Bencher) {
    bench_many_hash(b, 1 * KIB);
}

#[bench]
fn bench_many_hash_0001_kib_loop(b: &mut Bencher) {
    bench_many_hash_loop(b, 1 * KIB);
}

// Past one chunk, so the batch recurses and builds a tree per input.
#[bench]
fn bench_many_hash_0004_kib(b: &mut Bencher) {
    bench_many_hash(b, 4 * KIB);
}

#[bench]
fn bench_many_hash_0004_kib_loop(b: &mut Bencher) {
    bench_many_hash_loop(b, 4 * KIB);
}
